...
export class IntegrationService {
  ...
  constructor(loadBalancerHost: string) {
    this.gatewayService = new GatewayProxyService(loadBalancerHost);
  }
  ...
  validateMerchantInfo(
    ...
    if (!name || !email || !iban || !bic) {
      throw new InvalidMerchantInformationException(
        'Name, email, IBAN, and BIC are required.',
      );
    }
  ...
  validateApplicationInfo(
    ...
    if (!name || !email || !url || !description || !merchantId) {
      throw new InvalidApplicationInformationException(
  ...
  async integrateApplication(
    ...
    try {
      ...
      return await this.gatewayService.integrateApplication(
  ...
  async integrateService(application: {
    name: string;
    email: string;
    IBAN: string;
    BIC: string;
    ...
    try {
      const merchantIntegrationResult: MerchantDTO =
        await this.integrateMerchant(
          application.name,
          application.email,
          application.IBAN,
          application.BIC,
      ...
      console.log(
        `Merchant integration successful for ${merchantIntegrationResult.name}. Merchant ID: ${merchantIntegrationResult.id}`,
      );
      const applicationIntegrationResult: ApplicationDto =
        await this.integrateApplication(
      ...
      console.log(
        `Application integration successful for ${application.name}. Application ID: ${applicationId}`,
      );
